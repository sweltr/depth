...
for (i in me_objs) {
  ...
  tmp_final_rc <- data.frame(tmp_final$sample_sums()) %>% 
    ...
    dplyr::rename("curate_rc" = 2)
  
  tmp_rc <- readr::read_delim(paste0("files/CURATE/", tmp_name, "_sample_data_with_rc.txt"), delim = "\t")
  tmp_rc <- tmp_rc[, -c(2:6, 10,11)]
  tmp_summ <- dplyr::full_join(tmp_rc, tmp_final_rc)
  readr::write_delim(tmp_summ, paste0("files/SHARE/", tmp_name, "_track_read_counts.txt"), delim = "\t")

...
{{< downloadthis files/CUTADAPT/1_cut_ssu.R dname="1_cut_ssu" label="R Script for cutadapt" icon="code-slash" type="link" >}} primer removal from the SSU dataset. 
...
knitr::include_graphics("files/CUTADAPT/ssu_plot_qscores_raw.png")
```
...
knitr::include_graphics("files/CUTADAPT/ssu_plot_qscores_cut.png")
```
...
#| echo: false
#| eval: false
temp1 <- read.table("working_files/CUTADAPT/ssu/cutadapt_track.txt", header = TRUE)
temp1 <- dplyr::select(temp1, 1, 2, 6)

colnames(temp1)[2] <- "raw_reads"
colnames(temp1)[3] <- "cutadapt_reads"
		
write.table(temp1, "files/CUTADAPT/ssu_cutadapt_track.txt", sep = "\t", quote = FALSE, row.names = FALSE)
```

```{r}
#| echo: false
#| eval: true
seq_table <- read.table("files/CUTADAPT/ssu_cutadapt_track.txt",
                       header = TRUE, sep = "\t")
